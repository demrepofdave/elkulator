...
// Called when IDM_SETTINGS_MISC_SCREENSHOT event is recieved.
elk_event_t menu_handle_screenshot(ALLEGRO_EVENT * event)
{
    ALLEGRO_PATH *path = menu_save_gui(event, "Save screenshot", "*.snp", screenshot_fn);
...
ALLEGRO_PATH *movie_fn = NULL;

// Called when IDM_SETTINGS_MISC_START_MOVIE event is recieved.
elk_event_t menu_handle_startmovie(ALLEGRO_EVENT * event)
{
    ALLEGRO_PATH *path = menu_save_gui(event, "Save movie", "*.vid", movie_fn);
    ...
    if(path)
    {
        ...
        {
            log_debug("path_str='%s'", path_str);
            callback_handlers.handle_startmovie(path_str);
            if(movie_fn)
            {
                al_destroy_path(movie_fn);
                movie_fn = path;
            }
        } 
        else
...
// Called when IDM_SETTINGS_MISC_STOP_MOVIE event is recieved.
elk_event_t menu_handle_stopmovie (ALLEGRO_EVENT * event)
{
